...
FLOps provides a mock data provider (**MDP**) service _(container image)_ that helps newcomers or those without access to edge devices to get started quickly and use FLOps, e.g., on a single machine.
...
The `oak-cli` provides predefined MDP SLAs and can deploy them for you with a single [command](../../../../manuals/cli/features/flops/#oak-addon-flops-mock-data-m).
{{< /callout >}}
...
Once training starts _(assuming the data tags match)_, the learner service will fetch the stored data and merge it back together.
