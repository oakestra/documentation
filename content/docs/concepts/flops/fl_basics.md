...
In FL, the server is frequently called an _aggregator_, and clients are called _learners_.
Using the terms _server_ and _client_ in FL is still common.
We use various components, including non-FL servers and clients.
Thus, we prefer to highlight FL components using _aggregators_ and _learners_.
{{< /callout >}}
