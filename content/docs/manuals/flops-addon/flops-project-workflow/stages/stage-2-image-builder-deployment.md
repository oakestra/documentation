...
The manager checks for images in the FLOps image registry (_part of the management suite_) that match the ML repository that was part of the requested project SLA.
...
If images for **FL Actors** _(learners & aggregators)_ using the requested ML repository are missing, the manager will create and deploy a single image-builder service.
...
- Deploying the large image builder service _(pulled size ~3GB)_ can take time.
  {{< /callout >}}
